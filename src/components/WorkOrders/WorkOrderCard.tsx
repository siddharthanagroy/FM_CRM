      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border status-${workOrder.status}`}>
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-500">
            <div className="flex items-center space-x-1">
              <span className="font-mono text-blue-600 font-medium">{workOrder.workOrderId}</span>
            </div>
        ...
        <div className="flex flex-col items-end space-y-2 ml-4">
          {/* Priority */}
          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border priority-${workOrder.priority}`}>
            {workOrder.priority === 'high' && <AlertTriangle className="h-3 w-3 mr-1" />}
            {workOrder.priority.toUpperCase()}
          </span>

          {/* Type */}
          <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getTypeColor(workOrder.type)}`}>
            {getTypeIcon(workOrder.type)}
            <span className="ml-1">{workOrder.type.toUpperCase()}</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-200">
        ...
        {workOrder.actualHours && (
          <div className="text-sm text-gray-600">
            Actual time: {workOrder.actualHours}h
          </div>
        )}
      </div>
